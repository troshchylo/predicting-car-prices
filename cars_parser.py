...
class CarsSpider(scrapy.Spider):
    ...
    car_attributes = ["Rok produkcji", "Przebieg", "Pojemnosc skokowa", "Rodzaj paliwa"]
    ...
    def parse(self, response, page, link):
        ...
        try:
            last_page = int(
                response.css('li[data-testid="pagination-list-item"]')[-1]
                .attrib["aria-label"]
                .split()[-1]
            )
        except IndexError:
        ...
        if page + 1 < last_page:
            ...
            yield scrapy.Request(
                ...
                callback=self.parse,
                meta={"dont_redirect": True, "handle_httpstatus_list": [302]},
                cb_kwargs={"page": page + 1, "link": link},
            )
    ...
    def parse_car_data(self, url, raw_data):
        ...
        values = [value.strip() for value in li_elements.css("::text").getall()]
        ...
        car_data["Cena"] = raw_data.css("span.ooa-1bmnxg7.evg565y11::text").get()
